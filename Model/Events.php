...
//    $this->needsInteractions = $this->pConfig->needsInteractions();
    ...
    public function putObsPageView($observer, $request)
    {
        ...
        try {
            ...
            $cust_id = $this->_findCustomerId();
            $cust_email = $this->_findCustomerEmail();
            $event = ['sentAt'=>time(), 'eventType'=>$eventType, 'eventValue'=>$eventValue,'properties'=>"{\"customerEmail\":\"$cust_email\",\"itemId\":\"$itemId\",\"itemType\":\"$itemType\", \"itemName\":\"$itemName\"}"];
            $eventsList = ['eventList'=>[$event], 'sessionId'=>"$sess_id", 'trackingId'=>$this->trackingId, 'userId'=>"$cust_id"];
